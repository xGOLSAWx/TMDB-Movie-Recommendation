    ...
    const fetchAllTrailers = async () => {
      ...
      await Promise.all(
        movies.map(async (m) => {
          try {
            const { data } = await axios.get(
              `https://api.themoviedb.org/3/movie/${m.id}/videos`,
              { params: { api_key: apiKey } }
            );
            const yt = data.results.find(
              (v) => v.site === 'YouTube' && v.type === 'Trailer'
            );
            if (yt) map[m.id] = `https://www.youtube.com/watch?v=${yt.key}`;
          } catch {}
        })
      );
      setTrailers(map);
    };
    if (movies.length) fetchAllTrailers();
  }, [movies]);
