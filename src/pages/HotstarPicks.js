...
const HotstarPicks = () => {
  ...
  const fetchHotstarPicks = async (pageNumber, type, genreId) => {
    try {
      const res = await axios.get(`https://api.themoviedb.org/3/discover/${type}`, {
        params: {
          api_key: process.env.REACT_APP_TMDB_API_KEY,
          with_watch_providers: 337, // Hotstar provider ID
          watch_region: 'US',
      ...
      console.error('Failed to fetch Hotstar picks:', error);
    }
  };
  ...
  useEffect(() => {
    fetchHotstarPicks(page, contentType, selectedGenre);
  ...
  return (
    <div className="px-6 py-8">
      <h1 className="text-4xl font-bold mb-6 text-white text-center">Disney+ Hotstar Picks</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
        {items.map((item) => (
          <Link
            ...
            aria-label={`View details for ${contentType === 'movie' ? item.title : item.name}`}
          >
